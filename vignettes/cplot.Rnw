...
<<fig01b,include=TRUE>>=
cplot(X,x=f.tot.y,y=f.tot.c,size=a.tot,color=factor(AF.nM),shape=factor(AF.nM),alpha=0.5,subset=t.frame==13)
@
...
<<fig05a,include=TRUE>>=
cplot(X,f.tot.y~t.frame,facets=~pos,geom="smooth",subset=pos%in%c(1,8,15,22,29),yzoom=c(0,6e6))
@
